...
def plot3d(ds_wine, ds_white, ds_red, features):

    x_w, y_w = (ds_white[features[0]], ds_white[features[1]])
    x_r, y_r = (ds_red[features[0]], ds_red[features[1]])

    xmin = min(ds_wine[features[0]])
    xmax = max(ds_wine[features[0]])
    ymin = min(ds_wine[features[1]])
    ymax = max(ds_wine[features[1]])
    
    X, Y = np.mgrid[xmin:xmax:100j, ymin:ymax:100j]
    positions = np.vstack([X.ravel(), Y.ravel()])
    values_w = np.vstack([x_w, y_w])
    kernel_w = gaussian_kde(values_w)
    #pdf_w = kernel_w.pdf(positions)
    Z_w = np.reshape(kernel_w(positions).T, X.shape)

    values_r = np.vstack([x_r, y_r])
    kernel_r = gaussian_kde(values_r)
    #pdf_r = kernel_r.pdf(positions)
    Z_r = np.reshape(kernel_r(positions).T, X.shape)

    fig = plt.figure(figsize=(20,20))
    ax = fig.add_subplot(1, 2, 1, projection= '3d')

    for i, (elev, anim, title) in enumerate(zip([10, 10, 25, 10], 
                                                [0, -90, 25, 100],
                                                ['x axis', 'y axis', 'view 1', 'view 2']
                                                )):
        # Plot the surface.
        surf = ax.plot_surface(X, Y, Z_w, cmap=plt.cm.coolwarm,linewidth=0, antialiased=False, alpha=.7)
        surf = ax.plot_surface(X, Y, Z_r, cmap=plt.cm.binary,linewidth=0, antialiased=False, alpha=.7)
        #ax.scatter(x_innov, y_innov, zs=0, zdir='z', c='k')
        #ax.scatter(x_noninnov, y_noninnov, zs=0, zdir='z', c='k')
        ax.legend() 
        #ax.set(xlabel="TF/IDF score {}".format(features_innov[0]), ylabel="TF/IDF score {}".format(features_innov[1]),title="Densities of features {} and {} splitted on innovativeness".format(features_innov[0], features_innov[1]))
        #ax.suptitle("Densities of features {} and {} splitted on innovativeness".format(features_innov[0], features_innov[1]))
        ax.view_init(elev=elev, azim=anim)
    
    plt.show()
    return

    kde_x_innov = gaussian_kde(x_innov)
    kde_x_space_innov = np.linspace(x_innov.min(), x_innov.max(), 100)
    kde_x_eval_innov = kde_x_innov.evaluate(kde_x_space_innov)

    kde_y_innov = gaussian_kde(y_innov)
    kde_y_space_innov = np.linspace(y_innov.min(), y_innov.max(), 100)
    kde_y_eval_innov = kde_y_innov.evaluate(kde_y_space_innov)

    #__________________________________________________________________

    ds_noninnov = dataSet.loc[dataSet['Innov'] == 0, features_noninnov]
    ds_noninnov = ds_noninnov[(ds_noninnov !=0).all(1)]
    x_noninnov = ds_noninnov[features_noninnov[0]]
    y_noninnov = ds_noninnov[features_noninnov[1]]


    kde_x_noninnov = gaussian_kde(x_noninnov)
    kde_x_space_noninnov = np.linspace(x_noninnov.min(), x_noninnov.max(), 100)
    kde_x_eval_noninnov = kde_x_noninnov.evaluate(kde_x_space_noninnov)

    kde_y_noninnov = gaussian_kde(y_noninnov)
    kde_y_space_noninnov = np.linspace(y_noninnov.min(), y_noninnov.max(), 100)
    kde_y_eval_noninnov = kde_y_noninnov.evaluate(kde_y_space_noninnov)
    #___________________________________________________________________

    # Grid of x and y
    X_innov, Y_innov = np.meshgrid(kde_x_space_innov, kde_y_space_innov)
    # Grid of probability density
    kX_innov, kY_innov = np.meshgrid(kde_x_eval_innov, kde_y_eval_innov)
    # Intersection
    Z_innov = kX_innov * kY_innov

    X_noninnov, Y_noninnov = np.meshgrid(kde_x_space_noninnov, kde_y_space_noninnov)
    # Grid of probability density
    kX_noninnov, kY_noninnov = np.meshgrid(kde_x_eval_noninnov, kde_y_eval_noninnov)
    # Intersection
    Z_noninnov = kX_noninnov * kY_noninnov
    ...
    for i, (elev, anim, title) in enumerate(zip([10, 10, 25, 10], 
        ...
        # Plot the surface.
        surf = ax.plot_surface(X_innov, Y_innov, Z_innov, cmap=plt.cm.coolwarm,linewidth=0, antialiased=False, alpha=.7)
        #surf = ax.plot_surface(X_noninnov, Y_noninnov, Z_noninnov, cmap=plt.cm.binary,linewidth=0, antialiased=False, alpha=.7)
        #ax.scatter(x_innov, y_innov, zs=0, zdir='z', c='k')
        #ax.scatter(x_noninnov, y_noninnov, zs=0, zdir='z', c='k')
        ax.legend() 
        ax.set(xlabel="TF/IDF score {}".format(features_innov[0]), ylabel="TF/IDF score {}".format(features_innov[1]),title="Densities of features {} and {} splitted on innovativeness".format(features_innov[0], features_innov[1]))
...
overlap_MC1D = [0.6022148758235613,
              0.3497053598351171,
              0.6285634219699338,
              0.3844249291766734,
              0.25224822287749504,
...
misclassification = [0.2367397490400387,
                     0.16605529216339282,
                     0,
                     0,
                     0.10732085245091365,
